...
! 		end if
	...
	Subroutine sch_scheme_angle(this, f, fprev, g)

		Class(sch) :: this
		Class(grid) :: g

		Integer(4) :: x, y
		Real(8), Intent(inout) :: f(g.ns_y: g.nf_y, g.ns_x-g.bstep: g.nf_x+g.fstep)
		Real(8), Intent(inout) :: fprev(g.ns_y: g.nf_y, g.ns_x-g.bstep: g.nf_x+g.fstep)
! 		Real(8) :: Gamma_x, Gamma_y
! 		Gamma = g.Gamma
		do x=g.ns_x,g.nf_x
			do y=g.ns_y+1,g.nf_y
				f(y,x)=fprev(y,x)-g.Gamma_x*(fprev(y,x)-fprev(y,x-1))-g.Gamma_y*(fprev(y,x)-fprev(y-1,x))
			end do

! 			if ( this.eqvtype == 2 ) then        
! 				Gamma = (fprev(x)*g.dt)/g.dx
! 			end if
! 			f(x)=fprev(x)-Gamma*(fprev(x)-fprev(x-1))
! 			!y(x)=2
		end do
