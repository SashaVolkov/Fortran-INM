...
IMPLICIT NONE
	...
	Subroutine Lagr_init(this, lcase, iter_stpes, eqvtype, g)
		...
		Allocate(this.velocity(g.ns-g.bstep:g.nf+g.fstep))
		Allocate(this.xpoints(g.ns-g.bstep:g.nf+g.fstep))
	...
	Subroutine Lagr_SemiLagr(this, prev_mass, mass, g, interp)
		...
		Real(8) ::  delta, x_wave
		Integer(4) :: x, i, p
		Real(8), Intent(inout) :: prev_mass(g.ns - g.bstep : g.nf + g.fstep)
		Real(8), Intent(inout) :: mass(g.ns - g.bstep : g.nf + g.fstep)


		do x = g.ns - g.bstep, g.nf + g.fstep
			if ( this.eqvtype == 2 ) then
				this.velocity(x) = prev_mass(x)
			elseif ( this.eqvtype == 1 ) then
				this.velocity(x) = g.velocity
			end if
		end do
