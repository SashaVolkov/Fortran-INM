...
! 			Procedure :: deinit => met_deinit
	...
	Subroutine met_Print(this, f, g, t, name, timeset)
		...
		call MPI_Gather(W_mass(g.id*g.Xsize + 1), g.Xsize, MPI_DOUBLE_PRECISION, W_mass, g.Xsize,MPI_DOUBLE_PRECISION, 0, MPI_COMM_WORLD, this.ier)

		if ( g.id == 0 ) then
			print *, "Writing"
			open(14,file='/home/sasha/Fortran/Convection/datFiles/'//name)
			write(14,'(2(3x,e20.12))') (Real(x-1,8)*g.dx, W_mass(x), x=1,g.StepsX)
			close(14)
		end if
	...
	Subroutine met_func_BornParam(this, f, g)
		...
		this.Fmax = MAXVAL(f.d(g.ns: g.nf))
		this.Fmin = MINVAL(f.d(g.ns: g.nf))
