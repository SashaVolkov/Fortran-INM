...
CONTAINS
	...
	subroutine init(this, dim, step, omega_cor, r_sphere, g, dt)
		...
		real(8) grid_points(1:2, -dim:dim, -dim:dim)
	...
	print '(" Allocate")'
		...
		call generate.conformal_cubed_sphere(dim, dim, r_sphere, this.grid_points_ll)
		call this.const_def()
		call this.step_min()
	...
	subroutine alloc(this)
		...
		Allocate(this.grid_points_ll(1:2, -this.dim:this.dim, -this.dim:this.dim, 1:6))
...
! 		real(8), intent(in) :: grid_points(1:2, -this.dim:this.dim, -this.dim:this.dim, 1:6)
		...
		do face_idx = 1, 6 ! Only longitude
			do y = -dim, dim
				do x = -dim, dim
					this.f_cor(x, y, face_idx)= 2*omega_cor*dsin(this.grid_points_ll(2, x, y, face_idx))
				end do
			end do
		end do
...
if(y+1 > dim)then ! y
call distance_sphere(r_sphere, this.grid_points_ll(:, y, x, face_idx), this.grid_points_ll(:, y-1, x, face_idx), dist)
this.h_dist(1, y, x) = dist
else
call distance_sphere(r_sphere, this.grid_points_ll(:, y+1, x, face_idx), this.grid_points_ll(:, y, x, face_idx), dist)
this.h_dist(1, y, x) = dist
end if

if(x+1 > dim)then ! x
call distance_sphere(r_sphere, this.grid_points_ll(:, y, x, face_idx), this.grid_points_ll(:, y, x-1, face_idx), dist)
this.h_dist(2, y, x) = dist
else
call distance_sphere(r_sphere, this.grid_points_ll(:, y, x+1, face_idx), this.grid_points_ll(:, y, x, face_idx), dist)
this.h_dist(2, y, x) = dist
end if
