...
Subroutine FRunge(this, metr, var, i)
	...
	Real(8) :: G_inv(2,2), A(2,2), grad_Fx_con, grad_Fy_con, u0, S_c(2), S_cll(2), S_cor(2,2), S_p(2), S_pll(2), lat, lat_vel, lon_vel, h, u_cov, v_cov
...
! temp1(:) = this.ku_con(x, y-step:y+step, face, 0) + coef(i-1)*this.ku_con(x, y-step:y+step, face, i-1)
! temp2(:) = this.kv_con(x-step:x+step, y, face, 0) + coef(i-1)*this.kv_con(x-step:x+step, y, face, i-1)
vorticity = 0d0 !d.vorticity(metr, temp1, temp2, dh, x, y, step)
...
call metr.cov_to_con(grad_Fx,grad_Fy, S_p(1), S_p(2), x,y)
call metr.con_to_spherical(temp1(0),temp2(0), lon_vel, lat_vel, x,y,face)
call metr.spherical_to_con((var.f(x,y,face) + vorticity)*lat_vel, -(var.f(x,y,face) + vorticity)*lon_vel, S_c(1), S_c(2), x, y, face)
...
this.kh(x, y, face, i) = - dt*(height + h)*div - temp1(0)*dt*grad_Fx - temp2(0)*dt*grad_Fy
