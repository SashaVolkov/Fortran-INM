module printer_ncdf
	...
	subroutine init(this, dim, Tmax, speedup, time, Wid, xid, yid, faceid, ncid, rescale, grid_type)
		...
		if(grid_type == 0) then
			...
			if(rescale == 0) then
				status = nf90_create (path = trim('datFiles/'//"surface_conf_simple.nc"), &
		cmode = IOR(NF90_NETCDF4,IOR(NF90_MPIIO,NF90_CLOBBER)), comm = MPI_COMM_WORLD, info = MPI_INFO_NULL, ncid = ncid)
				...
				status = nf90_create (path = trim('datFiles/'//"surface_conf_tan.nc"), &
		cmode = IOR(NF90_NETCDF4,IOR(NF90_MPIIO,NF90_CLOBBER)), comm = MPI_COMM_WORLD, info = MPI_INFO_NULL, ncid = ncid)
			else if(rescale == 2) then
				status = nf90_create (path = trim('datFiles/'//"surface_conf_exp.nc"), &
		cmode = IOR(NF90_NETCDF4,IOR(NF90_MPIIO,NF90_CLOBBER)), comm = MPI_COMM_WORLD, info = MPI_INFO_NULL, ncid = ncid)
			end if

		else if(grid_type == 1) then

				status = nf90_create (path = trim('datFiles/'//"surface_equiang.nc"), &
		cmode = IOR(NF90_NETCDF4,IOR(NF90_MPIIO,NF90_CLOBBER)), comm = MPI_COMM_WORLD, info = MPI_INFO_NULL, ncid = ncid)
