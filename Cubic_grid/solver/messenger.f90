...
subroutine msg(this, f, paral)
	...
	call this.Halo_Rotations(paral, f.u_cov, 1, 0)
	call this.Halo_Rotations(paral, f.v_cov, 0, 1)
	...
	do face = 1, 6
		do i = 1, 4
			...
			if(paral.border(face, i) == 0 .or. paral.border(face, i) == 2) then ! coordinate rotation from one face to other if rotation 90 or -90 deg
				call MPI_IRecv(f.u_cov(rx, ry, face), 1, paral.halo(face, i), neib_id, rcv_tag, this.comm(2), this.rcv_req(2, i, face), this.ier) ! x -> x
				call MPI_IRecv(f.v_cov(rx, ry, face), 1, paral.halo(face, i), neib_id, rcv_tag, this.comm(3), this.rcv_req(3, i, face), this.ier) ! y -> y
			else
				call MPI_IRecv(f.u_cov(rx, ry, face), 1, paral.halo(face, i), neib_id, rcv_tag, this.comm(3), this.rcv_req(3, i, face), this.ier) ! x -> y
				call MPI_IRecv(f.v_cov(rx, ry, face), 1, paral.halo(face, i), neib_id, rcv_tag, this.comm(2), this.rcv_req(2, i, face), this.ier) ! y -> x
			end if
