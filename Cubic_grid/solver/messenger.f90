module messenger
	...
	integer(4) ier, snd_req(6, 4, 3), rcv_req(6, 4, 3)
...
subroutine Simple_msg(this, paral, f, face, i)
	...
	call MPI_IRecv(f.h_height(face, sx, sy), 1, paral.grey(i), neib_id, paral.id, mp_cw, this.rcv_req(face, i, 1), this.ier)
	call MPI_IRecv(f.u_vel(face, sx, sy), 1, paral.grey(i), neib_id, paral.id, mp_cw, this.rcv_req(face, i, 2), this.ier)
	call MPI_IRecv(f.v_vel(face, sx, sy), 1, paral.grey(i), neib_id, paral.id, mp_cw, this.rcv_req(face, i, 3), this.ier)

	call MPI_ISend(f.h_height(face, sx, sy), 1, paral.grey(i), neib_id, neib_id, mp_cw, this.snd_req(face, i, 1), this.ier)
	call MPI_ISend(f.u_vel(face, sx, sy), 1, paral.grey(i), neib_id, neib_id, mp_cw, this.snd_req(face, i, 2), this.ier)
	call MPI_ISend(f.v_vel(face, sx, sy), 1, paral.grey(i), neib_id, neib_id, mp_cw, this.snd_req(face, i, 3), this.ier)
